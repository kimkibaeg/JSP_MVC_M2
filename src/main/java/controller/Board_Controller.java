...
//			System.out.println("DB 저장 성공");
			...
			// 2. DAO 객체의 getBoardList(DTO)
			BoardDAO dao = new BoardDAO();
			
			// 리턴 받을 변수 선언
			List<BoardDTO> boardList = new ArrayList<>();
			
			// boardList : DB의 Board 테이블의 레코드를 dto 로 저장 후 ArrayList 안의 각 방에 저장된 상태
			...
			// 1. Client 넘어오는 파라미터 SEQ 변수이 값을 읽어서 DTO 에 저장 후 dao.getBoard(DTO)
			// http://localhost:8181/JSP_MVC_M2/getBoard.do?seq=5
			
			int seq = Integer.parseInt(request.getParameter("seq"));
			
			// 2. DTO 에 SEQ 값을 setter 주입
			BoardDTO dto = new BoardDTO();
			dto.setSeq(seq);
